...
A disposable Python 3.13 fixture verified:

- CLI discovery and empty-state JSON;
- safe command execution in an isolated working directory;
- queue event logging;
- deterministic FIFO serialization;
- timeout termination with exit code `124`;
- in-memory FastMCP transport;
- discovery of `run_task` and `clear_task_logs`;
- a successful non-shell MCP tool call.

The FIFO test initially used fixed shell-launch timing and exposed a harness race. The final test waits for an explicit first-task-start marker before submitting task two. This proves queue ordering rather than process startup ordering.
...
- Test policy denial, secret filtering, path boundaries, cancellation and nested process-tree termination through the Amarax adapter.
